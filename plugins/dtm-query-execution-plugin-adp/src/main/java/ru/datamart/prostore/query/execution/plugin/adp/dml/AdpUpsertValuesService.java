...
import static ru.datamart.prostore.query.execution.plugin.api.dml.LlwUtils.getExtendRowsOfValues;

@Service("adpUpsertValuesService")
public class AdpUpsertValuesService implements UpsertValuesService {
    ...
    private final AdpUpsertDataTransferService dataTransferService;

    public AdpUpsertValuesService(@Qualifier("adpSqlDialect") SqlDialect sqlDialect,
                                  DatabaseExecutor executor,
                                  AdpUpsertDataTransferService dataTransferService) {
    ...
    private SqlNode getStagingIdentifier(UpsertValuesRequest request) {
        return identifier(request.getDatamartMnemonic(), request.getEntity().getName() + "_" + Constants.STAGING_TABLE);
    }
}
