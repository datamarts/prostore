...
public class AdpStandaloneQueriesFactory {
    private static final String CREATE_STANDALONE_PATTERN = "CREATE TABLE %s (%s, PRIMARY KEY (%s))";
    private static final String DROP_STANDALONE_PATTERN = "DROP TABLE %s";

    private AdpStandaloneQueriesFactory() {
    }
    ...
    public static String createQuery(Entity entity) {
        ...
        val columnList = columns.stream()
                ...
                .map(field -> String.format("%s %s %s", field.getName(),
                        AdpTypeUtil.adpTypeFromDtmType(field),
                        field.getNullable() ? "" : "NOT NULL"))
                .collect(Collectors.joining(", "));
        return String.format(CREATE_STANDALONE_PATTERN, entity.getExternalTableLocationPath(), columnList, pkFields);
    }

    public static String dropQuery(String externalLocationPath) {
        return String.format(DROP_STANDALONE_PATTERN, externalLocationPath);
    }
}
