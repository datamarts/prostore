...
public class AdbDeleteService implements DeleteService {
    private final AdbLogicalDeleteService adbLogicalDeleteService;
    private final AdbStandaloneDeleteService adbStandaloneDeleteService;


    public AdbDeleteService(AdbLogicalDeleteService adbLogicalDeleteService,
                            AdbStandaloneDeleteService adbStandaloneDeleteService) {
        this.adbLogicalDeleteService = adbLogicalDeleteService;
        this.adbStandaloneDeleteService = adbStandaloneDeleteService;
    }

    @Override
    public Future<Void> execute(DeleteRequest request) {
        if (request.getEntity().getEntityType() == EntityType.WRITEABLE_EXTERNAL_TABLE) {
            return adbStandaloneDeleteService.execute(request);
        }

        return adbLogicalDeleteService.execute(request);
    }
}
