...
public class DropStandaloneTableExecutor implements EddlExecutor {
    ...
    @Autowired
    public DropStandaloneTableExecutor(@Qualifier("adbQueryExecutor") DatabaseExecutor adbQueryExecutor) {
    ...
    public Future<Void> execute(EddlRequest request) {
        return Future.future(promise -> {
            val sql = AdbStandaloneQueriesFactory.dropQuery(request.getEntity().getExternalTableLocationPath());
