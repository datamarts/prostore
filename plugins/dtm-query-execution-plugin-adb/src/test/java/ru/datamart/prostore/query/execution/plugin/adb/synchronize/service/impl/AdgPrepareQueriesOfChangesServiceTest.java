...
class AdgPrepareQueriesOfChangesServiceTest {
    ...
    void shouldSuccessWhenOneTable(VertxTestContext ctx) {
        ...
        // assert
        String expectedNewQuery = "SELECT dates.id, CAST(EXTRACT(EPOCH FROM dates.col_timestamp) * 1000000 AS BIGINT), CAST(EXTRACT(EPOCH FROM dates.col_date) / 86400 AS BIGINT), CAST(EXTRACT(EPOCH FROM dates.col_time) * 1000000 AS BIGINT), dates.col_boolean\n" +
    ...
    void shouldSuccessWhenOneTableAndNotNullColumns(VertxTestContext ctx) {
        ...
        // assert
        String expectedNewQuery = "SELECT dates.id, dates.col_boolean, CAST(EXTRACT(EPOCH FROM dates.col_time) * 1000000 AS BIGINT)\n" +
                "FROM datamart1.dates AS dates";
        String expectedDeletedQuery = "SELECT dates.id, dates.col_boolean\n" +
                "FROM datamart1.dates AS dates";
    ...
    void shouldSuccessWhenMultipleComplexTables(VertxTestContext ctx) {
        ...
        // assert
        String expectedNewFreshQuery = "SELECT t0.id, CAST(EXTRACT(EPOCH FROM t0.col_timestamp) * 1000000 AS BIGINT), CAST(EXTRACT(EPOCH FROM t0.col_date) / 86400 AS BIGINT), CAST(EXTRACT(EPOCH FROM t0.col_time) * 1000000 AS BIGINT), t0.name, t3.surname, t0.col_boolean\n" +
        ...
        String expectedNewStaleQuery = "SELECT t0.id, CAST(EXTRACT(EPOCH FROM t0.col_timestamp) * 1000000 AS BIGINT), CAST(EXTRACT(EPOCH FROM t0.col_date) / 86400 AS BIGINT), CAST(EXTRACT(EPOCH FROM t0.col_time) * 1000000 AS BIGINT), t0.name, t3.surname, t0.col_boolean\n" +
                "FROM (SELECT t1.id, t1.col_timestamp, t1.col_date, t1.col_time, t2.name, t1.col_boolean\n" +
                "FROM datamart1.dates AS t1\n" +
                "INNER JOIN datamart1.names AS t2 ON t1.id = t2.id) AS t0\n" +
                "INNER JOIN datamart1.surnames AS t3 ON t0.id = t3.id";
