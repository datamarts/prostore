...
import ru.datamart.prostore.query.execution.plugin.adb.synchronize.service.impl.AdqmPrepareQueriesOfChangesService;
...
class AdqmSynchronizeDestinationExecutorComplexTest {
    ...
    void shouldCorrectlySynchronizeWhenOneTable(VertxTestContext testContext) {
        ...
        result.onComplete(ar -> {
            ...
            testContext.verify(() -> {
                verify(databaseExecutor, times(8)).executeUpdate(stringArgumentCaptor.capture());
                List<String> allInvocations = stringArgumentCaptor.getAllValues();
                assertThat(allInvocations.get(0))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_matview");
                ...
                assertThat(allInvocations.get(5))
                        .isEqualToIgnoringNewLines("INSERT INTO datamart1.CLICKHOUSE_EXT_matview (id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, col_date, col_time, col_timestamp, col_boolean, col_uuid, col_link, sys_op, sys_from, sys_to, sys_close_date, sign) SELECT *, 0, 2, 9223372036854775807, 9223372036854775807, 1 FROM (SELECT id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, CAST(EXTRACT(EPOCH FROM col_date) / 86400 AS BIGINT) AS EXPR__8, CAST(EXTRACT(EPOCH FROM col_time) * 1000000 AS BIGINT) AS EXPR__9, CAST(EXTRACT(EPOCH FROM col_timestamp) * 1000000 AS BIGINT) AS EXPR__10, CASE WHEN col_boolean IS NOT NULL THEN CASE WHEN col_boolean THEN 1 ELSE 0 END ELSE NULL END AS EXPR__11, col_uuid, col_link FROM datamart1.tbl1_actual WHERE sys_from >= 0 AND sys_from <= 2) as __temp_tbl");
                assertThat(allInvocations.get(6))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_matview");
                assertThat(allInvocations.get(7))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_PK_matview");
    ...
    void shouldCorrectlySynchronizeWhenMultipleTables(VertxTestContext testContext) {
        ...
        result.onComplete(ar -> {
            ...
            testContext.verify(() -> {
                ...
                assertThat(allInvocations.get(5))
                        .isEqualToIgnoringNewLines("INSERT INTO datamart1.CLICKHOUSE_EXT_matview (id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, col_date, col_time, col_timestamp, col_boolean, col_uuid, col_link, sys_op, sys_from, sys_to, sys_close_date, sign) SELECT *, 0, 2, 9223372036854775807, 9223372036854775807, 1 FROM (SELECT t0.id, t0.col_varchar, t0.col_char, t0.col_bigint, t0.col_int, t2.col_int32, t2.col_double, t2.col_float, CAST(EXTRACT(EPOCH FROM t2.col_date) / 86400 AS BIGINT) AS EXPR__8, CAST(EXTRACT(EPOCH FROM t2.col_time) * 1000000 AS BIGINT) AS EXPR__9, CAST(EXTRACT(EPOCH FROM t2.col_timestamp) * 1000000 AS BIGINT) AS EXPR__10, CASE WHEN t2.col_boolean IS NOT NULL THEN CASE WHEN t2.col_boolean THEN 1 ELSE 0 END ELSE NULL END AS EXPR__11, t2.col_uuid, t2.col_link FROM (SELECT id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, col_date, col_time, col_timestamp, col_boolean, col_uuid, col_link FROM datamart1.tbl1_actual WHERE sys_from <= 2 AND COALESCE(sys_to, 9223372036854775807) >= 2) AS t0 INNER JOIN (SELECT id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, col_date, col_time, col_timestamp, col_boolean, col_uuid, col_link FROM datamart1.tbl2_actual WHERE sys_from <= 2 AND COALESCE(sys_to, 9223372036854775807) >= 2) AS t2 ON t0.col_bigint = t2.col_bigint EXCEPT SELECT t0.id, t0.col_varchar, t0.col_char, t0.col_bigint, t0.col_int, t2.col_int32, t2.col_double, t2.col_float, CAST(EXTRACT(EPOCH FROM t2.col_date) / 86400 AS BIGINT) AS EXPR__8, CAST(EXTRACT(EPOCH FROM t2.col_time) * 1000000 AS BIGINT) AS EXPR__9, CAST(EXTRACT(EPOCH FROM t2.col_timestamp) * 1000000 AS BIGINT) AS EXPR__10, CASE WHEN t2.col_boolean IS NOT NULL THEN CASE WHEN t2.col_boolean THEN 1 ELSE 0 END ELSE NULL END AS EXPR__11, t2.col_uuid, t2.col_link FROM (SELECT id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, col_date, col_time, col_timestamp, col_boolean, col_uuid, col_link FROM datamart1.tbl1_actual WHERE sys_from <= -1 AND COALESCE(sys_to, 9223372036854775807) >= -1) AS t0 INNER JOIN (SELECT id, col_varchar, col_char, col_bigint, col_int, col_int32, col_double, col_float, col_date, col_time, col_timestamp, col_boolean, col_uuid, col_link FROM datamart1.tbl2_actual WHERE sys_from <= -1 AND COALESCE(sys_to, 9223372036854775807) >= -1) AS t2 ON t0.col_bigint = t2.col_bigint) as __temp_tbl");
                assertThat(allInvocations.get(6))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_matview");
                assertThat(allInvocations.get(7))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_PK_matview");
    ...
    void shouldCorrectlyUseExceptWhenGroupByAggregate(VertxTestContext testContext) {
        ...
        result.onComplete(ar -> {
            ...
            testContext.verify(() -> {
                ...
                assertThat(allInvocations.get(5))
                        .isEqualToIgnoringNewLines("INSERT INTO datamart1.CLICKHOUSE_EXT_matview (id, sum, sys_op, sys_from, sys_to, sys_close_date, sign) SELECT *, 0, 2, 9223372036854775807, 9223372036854775807, 1 FROM (SELECT 1 AS EXPR__0, SUM(id) AS EXPR__1 FROM datamart1.tbl1_actual WHERE sys_from <= 2 AND COALESCE(sys_to, 9223372036854775807) >= 2 GROUP BY id EXCEPT SELECT 1 AS EXPR__0, SUM(id) AS EXPR__1 FROM datamart1.tbl1_actual WHERE sys_from <= -1 AND COALESCE(sys_to, 9223372036854775807) >= -1 GROUP BY id) as __temp_tbl");
                assertThat(allInvocations.get(6))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_matview");
                assertThat(allInvocations.get(7))
                        .isEqualToIgnoringNewLines("DROP EXTERNAL TABLE IF EXISTS datamart1.CLICKHOUSE_EXT_PK_matview");
    ...
    private Datamart prepareDatamart(String viewQuery) {
        ...
        Entity matView = Entity.builder()
                ...
                .destination(EnumSet.of(SourceType.ADQM))
