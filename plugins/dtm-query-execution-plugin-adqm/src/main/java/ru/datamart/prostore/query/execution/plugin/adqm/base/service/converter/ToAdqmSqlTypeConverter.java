...
@Component("toAdqmSqlTypeConverter")
public class ToAdqmSqlTypeConverter implements SqlTypeConverter {

    private final Map<ColumnType, Map<Class<?>, ColumnTransformer>> transformerMap;

    public ToAdqmSqlTypeConverter() {
        Map<ColumnType, Map<Class<?>, ColumnTransformer>> transformerMap = new HashMap<>();
        val numberFromLongTransformerMap = ColumnTransformer.getTransformerMap(new NumberFromLongTransformer());
        ...
        transformerMap.put(ColumnType.DATE, ColumnTransformer.getTransformerMap(new LongDateFromIntTransformer()));
        transformerMap.put(ColumnType.TIME, ColumnTransformer.getTransformerMap(new LongTimeFromLongTransformer()));
        transformerMap.put(ColumnType.TIMESTAMP, ColumnTransformer.getTransformerMap(new LongTimestampFromLongTransformer()));
