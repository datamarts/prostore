...
public class DropStandaloneTableExecutor implements EddlExecutor {
    ...
    private final AdqmStandaloneQueriesFactory queriesFactory;

    @Autowired
    public DropStandaloneTableExecutor(@Qualifier("adqmQueryExecutor") DatabaseExecutor databaseExecutor,
                                       AdqmStandaloneQueriesFactory queriesFactory) {
    ...
    public Future<Void> execute(EddlRequest request) {
        return Future.future(promise -> {
            val locationPath = request.getEntity().getExternalTableLocationPath();
            val dropDistributedSql = queriesFactory.dropDistributedTable(locationPath);
            val dropShardSql = queriesFactory.dropShardTable(locationPath);
            databaseExecutor.executeUpdate(dropDistributedSql)
                    .compose(ignore -> databaseExecutor.executeUpdate(dropShardSql))
                    .onComplete(promise);
        });
    }
}
