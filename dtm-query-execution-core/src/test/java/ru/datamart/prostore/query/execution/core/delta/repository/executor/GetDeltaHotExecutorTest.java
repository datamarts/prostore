...
class GetDeltaHotExecutorTest {
    ...
    public static final HotDelta DELTA_HOT = HotDelta.builder()
            .rollingBack(false)
            .deltaNum(1L)
            .cnFrom(2L)
            .cnTo(4L)
    ...
    private final GetDeltaHotExecutor deltaExecutor = new GetDeltaHotExecutor(zkExecutor, ENV);
    ...
    void shouldSucceed(VertxTestContext testContext) {
        ...
        deltaExecutor.execute(DATAMART)
                .onComplete(ar -> testContext.verify(() -> {
                    ...
                    assertEquals(DELTA_HOT, ar.result());
                }).completeNow());
    }
    @Test
    ...
    void shouldFailedDeserialization(VertxTestContext testContext) {
        //arrange
        when(zkExecutor.getData(anyString())).thenReturn(Future.succeededFuture(CoreSerialization.serialize(DELTA_HOT)));
